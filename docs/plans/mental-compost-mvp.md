...
- Use local Markdown files as the source of truth for entries.
- Keep the run script staging a small `.app` bundle from SwiftPM output.

## Core Pieces

- `DailyEntry`: one writing day, word count, goal state, timestamps, optional export timestamps.
- `CompostReview`: retained experimental review model for future optional reflection work.
...
- `CompostReviewStore`: retained storage for future optional reflection work.
...
- `ExportPathResolver` and `AIContextExporter`: choose export paths and write selected context only.
...
- Keep review/AI surfaces out of the primary writing flow until they are clearly useful.
